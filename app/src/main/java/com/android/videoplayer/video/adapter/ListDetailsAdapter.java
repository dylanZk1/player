...
public class ListDetailsAdapter extends BaseMultiItemAdapter<OpenEyesIndexItemBean,ListPlayerHolder> {
    ...
    public ListDetailsAdapter(List<OpenEyesIndexItemBean> data) {
        ...
        mVideoHeight = ((ScreenUtils.getInstance(App.getInstance().getContext()).getScreenWidth() - ScreenUtils.getInstance(App.getInstance().getContext()).dpToPxInt(32f))/2) * 9 /16;
    }
    ...
    private void setItemVideo(final ListPlayerHolder viewHolder, OpenEyesIndexItemBean data, int position) {
        if(null!=data){
            ...
            PlayerUtils.getInstance().setOutlineProvider(viewHolder.findViewById(R.id.item_root_view),ScreenUtils.getInstance(App.getInstance().getContext()).dpToPxInt(5f));
