...
public class ListPlayerAdapter extends BaseMultiItemAdapter<OpenEyesIndexItemBean,ListPlayerHolder> {
    ...
    public ListPlayerAdapter(List<OpenEyesIndexItemBean> data,boolean autoPlay) {
        ...
        mVideoHeight = (ScreenUtils.getInstance(App.getInstance().getContext()).getScreenWidth() - ScreenUtils.getInstance(App.getInstance().getContext()).dpToPxInt(32f)) * 9 /16;
    ...
    private void setItemVideo(final ListPlayerHolder viewHolder, OpenEyesIndexItemBean data, int position) {
        if(null!=data){
            ...
            PlayerUtils.getInstance().setOutlineProvider(itemContainer,ScreenUtils.getInstance(App.getInstance().getContext()).dpToPxInt(3f));
            ...
            if(null!=consumption){
                ...
                item_collection.setText(ScreenUtils.getInstance(App.getInstance().getContext()).formatWan(consumption.getCollectionCount(),true));
                item_comment.setText((295+position)+"");
                item_give.setText((565+position)+"");
                item_share.setText(ScreenUtils.getInstance(App.getInstance().getContext()).formatWan(consumption.getShareCount(),true));
            }else{
