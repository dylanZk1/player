...
import com.android.videoplayer.danmu.DanMuActivity;
...
public class MainActivity extends BaseActivity {
                ...
                .request(new OnPermissionCallback() {
                    ...
                    public void onDenied(List<String> permissions, boolean never) {
                        if (never) {
                            ...
                            Toast.makeText(MainActivity.this,"拒绝授权，请授予权限",Toast.LENGTH_SHORT).show();
                            gotoPermissions();
                        }
                    }
                });
        ...
        adapter.setOnItemClickListener((view, position, itemId) -> {
            ...
            if(null!=view&&view.getTag() instanceof Menu){
                ...
                switch (menu.getId()) {
                    ...
                    case 19:
                        intent = new Intent(MainActivity.this, DanMuActivity.class);
                        break;
                }
                if(null!=intent){
                    startActivity(intent);
    ...
    private void startGlobalWindowPlayer() {
        ...
        WidgetFactory.bindDefaultControls(controller,false,true,this);
        controller.setTitle("任意界面开启一个悬浮窗窗口播放器");//视频标题(默认视图控制器横屏可见)
        //悬浮窗口播放默认开启自动吸附悬停，如需禁用请阅读多参方法参数说明
        boolean globalWindow = videoPlayer.startGlobalWindow(ScreenUtils.getInstance(App.getInstance().getContext()).dpToPxInt(3), Color.parseColor("#99000000"));
    ...
    private void startMiniWindowPlayer() {
        if(null==mVideoPlayer){
            ...
            ControlWindowView controWindowView=new ControlWindowView(this);//加载中、开始播放
...
//            mVideoPlayer.startWindow();
            ...
            int width = (PlayerUtils.getInstance().getScreenWidth(MainActivity.this)/2)+ScreenUtils.getInstance(App.getInstance().getContext()).dpToPxInt(30f);
...
//            mVideoPlayer.startWindow(PlayerUtils.getInstance().dpToPxInt(3f),Color.parseColor("#80000000"),true);
            //悬浮窗口播放默认开启自动吸附悬停，如需禁用请阅读多参方法参数说明
            mVideoPlayer.startWindow(width,height,startX,startY,ScreenUtils.getInstance(App.getInstance().getContext()).dpToPxInt(3f),Color.parseColor("#99000000"));//初始显示的位置并添加窗口颜色和圆角大小
    ...
    private void quitFullScreen(VideoPlayer mVideoPlayer){
        if(mVideoPlayer!=null){
            mVideoPlayer.toggleFullScreen();
            mVideoPlayer.getController().reStartDelayedRunnable();
        }
    }
    ...
    public void onBackPressed() {
        Log.d("mVideoPlayer",String.valueOf(mVideoPlayer));
        if(null!=mVideoPlayer && null!=videoPlayer){
            if(mVideoPlayer.isBackPressed()) {
                if (videoPlayer.getOrientation() == IMediaPlayer.ORIENTATION_LANDSCAPE) {
                    quitFullScreen(videoPlayer);
                    return;
                }
                super.onBackPressed();
            }else{
