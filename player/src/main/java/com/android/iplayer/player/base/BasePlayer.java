package com.android.iplayer.player.base;
...
import com.android.iplayer.player.interfaces.IBasePlayer;
import com.android.iplayer.base.interfaces.IPlayerControl;
import com.android.iplayer.base.interfaces.IRenderView;
import com.android.iplayer.base.listener.OnPlayerEventListener;
import com.android.iplayer.player.listener.OnWindowActionListener;
import com.android.iplayer.base.manager.IVideoManager;
import com.android.iplayer.player.manager.IWindowManager;
import com.android.iplayer.base.media.IMediaPlayer;
import com.android.iplayer.player.media.IVideoPlayer;
...
import com.android.iplayer.player.widget.view.ScreenOrientationRotate;
import com.android.iplayer.player.widget.view.WindowPlayerFloatView;
...
public abstract class BasePlayer extends FrameLayout implements IPlayerControl, IBasePlayer, ScreenOrientationRotate.OnDisplayOrientationListener {
    ...
    protected Boolean isOutLink = false;
    ...
    protected void setOrientation(int orientation){
        ...
        if(orientation == IMediaPlayer.ORIENTATION_PORTRAIT){
            ...
            //保存播放器在父布局中的宽、高、index层级等属性(如果存在的话)
            ...
            //给播放器添加一个背景颜色
            findViewById(R.id.player_surface).setBackgroundColor(Color.parseColor("#000000"));//设置一个背景颜色
        ...
        }else if(orientation == IMediaPlayer.ORIENTATION_LANDSCAPE){
            activity.setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_SENSOR_LANDSCAPE);
            setScreenOrientation(IMediaPlayer.ORIENTATION_LANDSCAPE);
        }
    }
...
//        ILogger.d(TAG,"progress-->currentDurtion:"+currentDurtion+",totalDurtion:"+totalDurtion);
    ...
    public void hideSystemBarStatus(Activity activity){
        if(activity == null){
            return;
        }
...
//        ILogger.d(TAG,"quitLandscapeScreen");
        ...
        if(null!=activity&&!activity.isFinishing()){
            ...
            //3.将自己交给此前的宿主ViewGroup,并还原播放器在原宿主的宽、高、index位置（要先判断父布局是否存在）
            if(null!=mParent && !isLocal){
                //3.还原全屏设置为正常设置
                ...
                onQuitFullScreen(false, mParent);
            }else{
                //由上层布局去执行退出全屏后的操作
                onQuitFullScreen(true, mParent);
            }
        }
    }
...
//                ILogger.d(TAG,"quitWindow-->窗口不存在");
            ...
            if(null!=mParent){
                ...
                onQuitWindow(false, mParent);
            }else{
                //通知宿主监听器触发返回事件，或由上层决定退出需要的操作
                onQuitWindow(true, mParent);
            }
        }
    }
...
//                ILogger.d(TAG,"startGlobalWindow-->上下文为空,开启失败");
    ...
    public abstract void onQuitFullScreen(boolean isLocal, ViewGroup viewGroup);

    public abstract void onQuitWindow(boolean isLocal, ViewGroup viewGroup);
}