...
public class IJkMediaPlayer extends AbstractMediaPlayer implements IMediaPlayer.OnPreparedListener,
    ...
    private void setOption(boolean isLive) {
        ...
        mMediaPlayer.setOption(IjkMediaPlayer.OPT_CATEGORY_PLAYER, "mediacodec-mpeg2", 1);
        mMediaPlayer.setOption(IjkMediaPlayer.OPT_CATEGORY_PLAYER, "mediacodec-mpeg4", 1);
                ...
                if (ContentResolver.SCHEME_ANDROID_RESOURCE.equals(uri.getScheme())) {
                    ...
                    if (headers != null&&headers.size()>0) {
                        ...
                        if(ContentResolver.SCHEME_CONTENT.equals(uri.getScheme())){
                            mMediaPlayer.setDataSource(getContext(), Uri.parse(ContentToString.getAbsolutePath(getContext(),uri)), headers);
                        }else{
                            mMediaPlayer.setDataSource(getContext(), uri, headers);
                        }
                    }else{
                        Uri uri2 = Uri.parse(path);
                        if(ContentResolver.SCHEME_CONTENT.equals(uri2.getScheme())){
                            mMediaPlayer.setDataSource(ContentToString.getAbsolutePath(getContext(),uri2));
                        }else{
