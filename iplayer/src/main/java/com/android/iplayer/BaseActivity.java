...
import com.android.iplayer.base.listener.BaseContract;
import com.android.iplayer.base.presenter.BasePresenter;
import com.android.iplayer.widget.listener.OnMenuActionListener;
...
import com.android.iplayer.widget.view.PlayerMenuDialog;
...
public abstract class BaseActivity <P extends BasePresenter> extends AppCompatActivity implements BaseContract.BaseView {
    ...
    public static final String MP4_URL1          ="https://vfx.mtime.cn/Video/2019/03/18/mp4/190318231014076505.mp4";//叶问1预告
    public static final String MP4_URL2          ="https://drobot-1300208283.cos.ap-nanjing.myqcloud.com/1683493772191174005-1905388747193831650.mp4";//2021款第七代天籁
    ...
    public static final String LIVE_RTMP2        ="rtmp://ns8.indexforce.com/home/mystream";//伊拉克 Al Sharqiya 电视台 (MediaPlayer(不支持)、IJKMediaPlayer(支持)、ExoPlayer(支持))
    public static final String LIVE_RTMP1        ="https://nhkw-zh-hlscomp.akamaized.net/8thz5iufork8wjip/playlist.m3u8";//韩国GOOD TV  (MediaPlayer(不支持)、IJKMediaPlayer(支持)、ExoPlayer(支持))
...
//                mMenuDialog = new PlayerMenuDialog(context, R.style.MenuButtomAnimationStyle);
...
//                }
//            });
            boolean globalWindow = mVideoPlayer.startGlobalWindow(ScreenUtils.getInstance(this).dpToPxInt(3), Color.parseColor("#99000000"));
    ...
    protected <T extends Dialog> void clearDialog(T dialog){
        if(dialog != null){
            dialog = null;
        }
    }
...
//        }
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            if(isInPictureInPictureMode() || (!isForbidCycle&&null!=mVideoPlayer)) mVideoPlayer.onDestroy();
        }else{
