 ...
 */
public abstract class AbstractControlWindowView extends BaseControlWidget implements View.OnClickListener {
    ...
    public AbstractControlWindowView(Context context) {
        super(context);
    }
    ...
    public void initViews() {
        ...
        mControllerStatus.setOnStatusListener(event -> {
...
//            IWindowManager.getInstance().onClickWindow();
    ...
    public void hideControl(boolean isAnimation) {
        if(isVisible()&&null!=mController){
            if(mController.getVisibility()!=View.GONE){
                if(isAnimation){
                    AnimationUtils.getInstance().startAlphaAnimatioTo(mController, getAnimationDuration(), false, animation -> mController.setVisibility(View.GONE));
                }else{
     ...
     */
    private final BaseHandel mExHandel=new BaseHandel(Looper.getMainLooper()){
    ...
    public abstract void onWindowFullScreen();
}