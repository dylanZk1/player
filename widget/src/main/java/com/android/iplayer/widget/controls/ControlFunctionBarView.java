...
import com.android.iplayer.base.orientation.OrientationData;
...
public class ControlFunctionBarView extends BaseControlWidget implements View.OnClickListener {
    ...
    private PlayerState playerState; //播放状态
    private String message;//状态信息
    private OrientationData orientationData;

    public ControlFunctionBarView(Context context, ViewModelStoreOwner owner) {
        super(context);
        if(owner!=null){
            orientationData = new ViewModelProvider(owner).get(OrientationData.class);
        }
    }

    @Override
    public int getLayoutId() {
        return R.layout.player_control_functionbar_portral;
    }
...
//                ILogger.d(TAG,"onStopTrackingTouch-->,isCompletion:"+isCompletion+",preViewTotalTime:"+mPreViewTotalTime);
                if(null!= mControlWrapper){
                    ...
                    mControlWrapper.danmuStart(seekBarProgress);
                    ILogger.d("getCurrentPosition", getCurrentPosition() +"  "+seekBarProgress);
//                    ILogger.d(TAG,"onStopTrackingTouch-->seekBarProgress:"+seekBarProgress+",ViewTotalTime:"+ mPreViewTotalTime +",duration:"+ mVideoPlayerControl.getDurtion()+getOrientationStr());
    ...
    public void hideControl(boolean isAnimation) {
        if(null!=mController){
            if(mController.getVisibility()!=View.GONE){
                if(isAnimation){
                    AnimationUtils.getInstance().startTranslateLocatToBottom(mController, getAnimationDuration(), animation -> {
    ...
    public void onOrientation(int direction) {
        ...
        if(IMediaPlayer.ORIENTATION_LANDSCAPE==direction){
            ViewSet(R.layout.player_control_functionbar);
            initViews();
            if(playerState!=null){
                onPlayerState(playerState,message);
                onPlayerScene(getPlayerScene());
                showSoundMute(orientationData != null && orientationData.getIsShowMute());
                if(orientationData.getIsShowMute()){
                    updateMute();
                }
            }
            ...
            ViewSet(R.layout.player_control_functionbar_portral);
            initViews();
            if(playerState!=null){
                onPlayerState(playerState,message);
                onPlayerScene(getPlayerScene());
    ...
    public void onMute(boolean isMute) {
        ImageView muteImage = findViewById(R.id.controller_btn_mute);
    ...
    private void updateMute(){
        if(null!= mControlWrapper){
            ...
            ImageView muteImge = findViewById(R.id.controller_btn_mute);
    ...
    public void showSoundMute(boolean showSound){
        orientationData.setIsShowMute(showSound);
        ImageView muteImage = findViewById(R.id.controller_btn_mute);
