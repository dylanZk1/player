...
public class ControlToolBarView extends BaseControlWidget implements View.OnClickListener {
    ...
    public void hideControl(boolean isAnimation) {
        if(null!=mController){
            if(mController.getVisibility()!=View.GONE){
                if(isAnimation){
                    AnimationUtils.getInstance().startTranslateLocatToTop(mController, getAnimationDuration(), animation -> mController.setVisibility(GONE));
                }else{
    ...
    private void setViewMarginTop(View view, int marginTop) {
    ...
    public void onOrientation(int direction) {
        ...
        if(IMediaPlayer.ORIENTATION_LANDSCAPE==direction){
            setViewMarginTop(mController, 0);
            show();
            ...
            //添加系统时间\电池电量组件
            FrameLayout controllerBattery = findViewById(R.id.controller_battery);
            if(controllerBattery != null && controllerBattery.getChildCount() == 0){
            ...
            setViewMarginTop(mController, ScreenUtils.getInstance(getContext()).getStatusBarHeight(getContext()));
            findViewById(R.id.controller_title).setVisibility(View.VISIBLE);//视频标题
            findViewById(R.id.controller_title_back).setVisibility(VISIBLE);//返回按钮
            findViewById(R.id.controller_menus).setVisibility(View.VISIBLE);//菜单栏(投屏\悬浮窗\菜单按钮)
            //移除系统时间\电池电量组件
//            FrameLayout controllerBattery = findViewById(R.id.controller_battery);
