package com.android.iplayer.base.utils;
...
import com.android.iplayer.base.widget.LayoutProvider;
...
//        ILogger.d(TAG,"isEdge-->eX:"+e.getRawX()+",eY:"+e.getRawY()+",screenWidt:"+getScreenWidth(context));
    ...
    public IControllerView[] remove(IControllerView[] array, int position) {
        int length = array.length;
        if (position < 0 || position > length) {
            throw new IndexOutOfBoundsException("the position is out of the array indices");
        }
        long startTime = System.currentTimeMillis();
        IControllerView[] newArray = new IControllerView[array.length - 1];
